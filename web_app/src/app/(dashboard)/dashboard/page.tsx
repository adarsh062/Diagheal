            ...
            <header className="space-y-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    ...
                    <div className="flex items-center gap-3">
                        <Link href="/dashboard/analyze" 
                            className="group flex items-center gap-3 px-8 py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 font-bold rounded-[1.5rem] hover:shadow-2xl transition-all active:scale-95 font-poppins text-xs uppercase tracking-widest">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {[
                        ...
                        <div key={stat.label} className="bg-white/80 dark:bg-slate-900/40 border border-white dark:border-slate-800 p-6 rounded-[2rem] shadow-sm flex items-center justify-between group hover:border-slate-200 transition-all">
                            <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <aside className="lg:col-span-4 space-y-8">
                    <div className="relative group bg-slate-900 dark:bg-white rounded-[2.5rem] p-8 text-white dark:text-slate-900 shadow-2xl overflow-hidden min-h-[320px] flex flex-col">
                        <div className="absolute top-0 right-0 w-32 h-32 bg-cyan-500/20 rounded-full blur-3xl -translate-y-16 translate-x-16" />
                            ...
                            {nextAppointment ? (
                                <div className="space-y-6 flex-grow">
                                    <div>
                                        ...
                                        <p className="text-slate-400 dark:text-slate-500 text-xs font-bold uppercase tracking-wider mt-1">{nextAppointment.specialty}</p>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="bg-white/5 dark:bg-slate-50 px-4 py-3 rounded-2xl border border-white/10">
                                        ...
                                        <div className="bg-white/5 dark:bg-slate-50 px-4 py-3 rounded-2xl border border-white/10 text-center">
                    ...
                    <div className="bg-white/50 dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 flex flex-col items-center text-center">
                        ...
                        <h4 className="font-poppins font-black text-sm uppercase tracking-widest">Health Insight</h4>
                        <p className="text-xs text-slate-500 dark:text-slate-400 font-barlow mt-2 leading-relaxed">
                            Consistent hydration and light cardio significantly improve liver regeneration markers.
                        </p>
                    </div>
                </aside>
            </div>

            <section className="bg-white border border-slate-100 dark:bg-slate-950 dark:border-slate-800 rounded-[3rem] p-10 shadow-2xl shadow-slate-100">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-12">
                    <div>
                        ...
                        <p className="text-slate-500 font-barlow text-sm mt-1">Review and manage your healthcare history.</p>
                    </div>
                    <Link href="/dashboard/history" className="px-6 py-3 border-2 border-slate-100 hover:bg-slate-900 hover:text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] transition-all font-poppins">
                        Full Archive
                    </Link>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                    {recentRecords.length === 0 ? (
                        <div className="col-span-full py-20 text-center bg-slate-50/50 dark:bg-slate-900/30 rounded-[2.5rem] border border-dashed border-slate-200">
                           <p className="font-poppins font-black text-slate-300 uppercase tracking-widest">No diagnostic history found</p>
                        </div>
                    ) : (
                        recentRecords.map((record: any) => { // Fixed with ': any' to resolve Vercel build error
                                ...
                                <div key={record.id} className="group p-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] hover:shadow-2xl transition-all flex flex-col">
                                    ...
                                    <div className="mt-auto grid grid-cols-2 gap-2">
                                        <Link href={`/dashboard/report/${record.id}`} 
                                            className="px-4 py-3 bg-slate-50 dark:bg-slate-800 hover:bg-slate-900 hover:text-white text-slate-900 dark:text-white rounded-2xl text-[10px] font-black uppercase tracking-widest text-center transition-all">
                                            View
                                        </Link>
                                        <Link 
                                            href={`/dashboard/report/${record.id}/print`}
                                            target="_blank"
                                            className="px-4 py-3 border border-slate-100 hover:border-slate-900 rounded-2xl text-[10px] font-black uppercase tracking-widest text-center transition-all text-slate-500">
                                            Print
                                        </Link>
                                    </div>
                                </div>
